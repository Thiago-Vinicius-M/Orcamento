export {
  getSupabaseClient as ensureSupabase,
  SUPABASE_NOT_CONFIGURED_MESSAGE,
} from '../lib/supabaseClient'

/** Em erros 4xx/5xx de `functions.invoke`, a mensagem útil pode vir no JSON de `error.context`. */
export async function getFunctionsErrorMessage(err: unknown): Promise<string | null> {
  ...
  try {
    ...
    // Se não for JSON, ainda tentamos extrair uma mensagem textual.
    try {
